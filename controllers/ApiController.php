...
use carono\exchange1c\interfaces\WarehouseInterface;
...
class ApiController extends Controller
{
    ...
    public function reloadClassifier($commerce)
    {
        if (isset($commerce->classifier->xml)) {
            $commerce->classifier->xml->saveXML($this->module->getTmpDir() . DIRECTORY_SEPARATOR . "classifier-{$commerce->classifier->id}.xml");
        } else {
            $classifierId = (string)($commerce->catalog ? $commerce->catalog->ИдКлассификатора : $commerce->offerPackage->ИдКлассификатора);
    ...
    public function parsingOffer($file)
    {
        ...
        if ($wareHouseClass = $this->getWarehouseClass()) {
            $wareHouseClass::createWarehouses(array_merge($commerce->offerPackage->getStockrooms(), $commerce->classifier->getStockrooms()));
        }
    ...
    public function actionImport($type, $filename)
    {
        ...
        switch ($type) {
            case 'catalog':
                ...
                if (mb_stripos($content, '<ПакетПредложений', 0, 'UTF-8')) {
    ...
    public function parseProductOffer($model, $offer)
    {
        ...
        $model->setRaw1cData($offer->owner, $offer);
        if (!$model->save()) {
     ...
     */
    protected function getWarehouseClass()
    {
        return $this->module->warehouseClass;
    }
